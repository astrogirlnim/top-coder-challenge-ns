...
def diminishing_receipts(receipts, days):
    """Apply diminishing returns to receipts based on trip duration"""
    if days >= 8:
...
def capped_mileage(miles, days):
    """Apply mileage caps based on trip duration"""
    if days == 1:
        first = min(miles, 300)
...
def main():
    ...
    # Formula (based on regression analysis and diminishing returns)
    base = 90 * days
